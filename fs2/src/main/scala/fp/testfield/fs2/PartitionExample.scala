...
final class FileOps[F[_]](val name: String)(implicit S: Sync[F]) {
  println(s"Open file: $name")

  def close(): F[Unit] = {
    S.delay(println(s"close file $name"))
  }
}
...
class PartitionPipe[F[_]](implicit S: Sync[F]) {
  ...
  private def go(inputStream: fs2.Stream[F, Record], exporter: Option[Exporter]): fs2.Pull[F, String, Unit] = {
    val tryPushResultAndReleaseExporter = exporter match {
      case Some(st) =>
        ...
        pushResult >> fs2.Pull.eval(S.delay(exporter.foreach(_.close())))
      case _ =>
        fs2.Pull.pure(())
    }
    ...
    inputStream.pull.uncons1.flatMap {
      ...
      case Some((record, tailStream)) =>
        val writeRecord = (state: Option[Exporter]) => fs2.Pull.eval(S.delay(state.foreach(_.write(record))))

        if (isTagChange(exporter, record.tag)) {
          val createExporter = fs2.Pull.eval(S.delay(Some(new Exporter(record.tag))))
...
object PartitionExample extends IOApp {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    fs2.Stream
      ...
      .evalMap(w => IO.delay(println(s"hdfs $w")))
      .compile
      .drain
